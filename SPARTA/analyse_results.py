...
for s_name in sat_names:
    for i_h, h in enumerate(hs):
        ...
        file_list = natsorted(glob.glob(sys.path[0]+"/SPARTA/setup/results_sparta/%s/force_%skm.*.gz" % (s_name, h)))
        ...
        if check_part_cells:
            file_list_pc = natsorted(glob.glob(sys.path[0]+"/SPARTA/setup/results_sparta/%s/npart_%skm.*.gz" % (s_name, h)))
