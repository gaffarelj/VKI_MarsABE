...
if __name__ == "__main__":
    ...
    for thrust_model in [2, 3]:
        satellites = SM.satellites if thrust_model == 3 else SM.satellites_with_tank
        ion_eff = [0.5, 0.25, 0.1, 0.01] if thrust_model == 3 else [1.0]
        for ionisation_efficiency in ion_eff:
            for use_battery in [False, True]:
                print(thrust_model, use_battery, ionisation_efficiency)
                ...
                run_opti = True:#(input("Run the optimisation ? ([y]/n) (if no, load latest saved results): ").lower().strip() in ["", "y"])
